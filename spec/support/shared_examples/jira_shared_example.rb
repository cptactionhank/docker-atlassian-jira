shared_examples 'an acceptable JIRA instance' do |database_examples|
  describe 'Going through the setup process' do
    ...
    context 'when processing application properties setup' do
      ...
      it { is_expected.to have_current_path %r{/secure/SetupProductBundle!default.jspa} }
      it { is_expected.to have_css '#jira-setupwizard' }
      it { is_expected.to have_css '#jira-setupwizard-product-selection-tracking' }
    end

    context 'when processing product bundle setup' do
      before :all do
        within '#jira-setupwizard' do
          find(:css, '#jira-setupwizard-product-selection-tracking').trigger('click')
          click_button 'Next'
        end
      end
